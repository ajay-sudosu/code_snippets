...
class DBClient:
    ...
    def get_lab_locations(self, zip_code, is_covid, is_insurance, test_name):
        ...
        elif (is_covid == 1 and test_name == "36 Hour RT-PCR") or (is_covid == 1 and test_name == "24 Hour RT-PCR"):
            final_data = covid_enzo_data
        else:
            final_data = covid_quest_data
        api_key = 'AIzaSyAe7nmOwtSLsOdfYwJ_ns3p1krP0h97rPE'
        k = requests.get(
            f"https://maps.googleapis.com/maps/api/geocode/json?address={zip_code}&key={api_key}")
        ...
        for lab in final_data[1::]:
            ...
            if is_insurance == 1 or "labcorp" not in lab[1].lower():
    ...
    def send_patient_message(self, name, doctor, phone, message_from):
        ...
        if message_from == "result":
            ...
            sms = f"NEXT MEDICAL BOOKING CONFIRMATION AND IMPORTANT INFO:\n\n\n1. If you have not already, fill out your intake form in your profile at joinnextmed.com/login. You will not be tested until you complete this process.\n\n2. You chose to use insurance for lab processing. Please fill in your insurance information on the intake form and present your insurance card at the lab. Failure to do so could lead to high out-of-pocket costs.\n\n3. Your visit begins with comprehensive testing at your chosen lab, followed by detailed results, post-testing treatment and consultation under the care of Dr. Marc Serota's medical team.\n\nIf you have any questions, please email us at team@joinnextmed.com or call 212-530-7870 and we will get back to you shortly."
    ...
    def consumer_add_patient(self, visit_date, visit_month, visit_year, patient_name, phone, address, current_date,
        ...
        x = (f'insert into visits (mrn, doctor_email, nurse_email, visit_status, visit_date, visit_month, visit_year, '
             ...
             f'is_complete_doctor, passport_number, is_approved, is_hiv, on_the_way, is_late, booking_date, booking_month, booking_year, booking_time, is_positive, location, number_of_lines, completed_timestamp, patient_symptoms, axle_patient, axle_address, pharmacy, patient_address, lab_fax, server_date_time,subscription_id,coupon,price) values ('
             f'"{mrn}", "{doctor_email}", "{nurse_email}", "{visit_status}", "{visit_date}", "{visit_month}", "{visit_year}", '
             f'"{visit_time}", "{patient_name}", "{email}", "{phone}", "{address}", "{notes}", "{dob_date}", "{dob_month}", '
             ...
             f'{live}, "{options}", "{race}", {is_family}, "{flight_time}", "{customer_id}", "{payment_id}", 0, 0, "{zip_code}", "{nurse_name}", {is_flight}, -1, -1, "", 0, "", 0, {is_hiv}, 0, 0, {current_date}, {current_month}, {current_year}, {current_time}, 0, "{location}", 0, "", "{patient_symptoms}", "{axle_patient}", "{axle_address}", "{pharmacy}", "{patient_address}", "{lab_fax}" ,"{dt_string}","{subscription_id}","{coupon}","{price}");')
    ...
    def get_not_verified_patients_from_visits(self):
        ...
        sql_query = """SELECT patient_name, phone, server_date_time
        FROM visits WHERE is_user_verified <> '1' """
        try:
    ...
    def insert_curexa_order(self, email, order_id):
        """insert into database"""
        try:
            self.connection.ping()
            with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                sql_query = """INSERT INTO curexa_orders (email, order_id) VALUES (%s, %s)"""
                cursor.execute(sql_query, (email, order_id))
            self.connection.commit()
            return {"status": "success"}
        except Exception as e:
            return {"status": "failure", "message": e}
    ...
    def get_curexa_order(self, email, order_id=None):
    ...
    def update_subscription_status(self, subscription_status, subscription_id):
        cursor = self.connection.cursor()
        x = f'update visits\net subscription_status = {subscription_status}\nwhere subscription_id = {subscription_id}'
        cursor.execute(x)
        self.connection.commit()
    ...
    def insert_gogomeds_order(self, AffiliateOrderNumber, GogomedAffiliateOrderNumber, GogomedOrderId):
        """insert into database"""
        try:
            self.connection.ping()
            with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                sql_query = """INSERT INTO gogomeds_orders (AffiliateOrderNumber, gogomed_affiliateordernumber, gogomed_orderid) VALUES (%s, %s, %s)"""
                cursor.execute(sql_query, (AffiliateOrderNumber, GogomedAffiliateOrderNumber, GogomedOrderId))
            self.connection.commit()
            return {"status": "success"}
        except Exception as e:
            return {"status": "failure", "message": e}

    def get_gogomeds_affiliateordernumber(self, AffiliateOrderNumber):
        try:
            self.connection.ping()
            with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                sql_query = f"""SELECT gogomed_affiliateordernumber FROM gogomeds_orders WHERE AffiliateOrderNumber='{AffiliateOrderNumber}' """
                print(sql_query)
                res = cursor.execute(sql_query)
                if res == 0:
                    return None
                result = dict(cursor.fetchone())
                return result.get("gogomed_affiliateordernumber", None)
        except Exception as e:
            return None
    ...
    def get_visits_by_patient_id_for_covermymeds(self, patient_id):
        try:
            self.connection.ping()
            with self.connection.cursor(pymysql.cursors.DictCursor) as cursor:
                sql_query = f"""SELECT * FROM visits WHERE patient_id={patient_id} """
                print(sql_query)
                res = cursor.execute(sql_query)
                if res == 0:
                    return None
                result = dict(cursor.fetchone())
                return result.get("patient_id", None)
        except Exception as e:
            return None

    # def get_visits_by_patient_id_covermymeds(self, patient_id):
