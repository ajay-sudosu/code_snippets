...
from mdintegrations import mdi_instance_dict, mdintegrations_chat, mdintegrations_api, \
    mdi_accutane, mdi_weightloss, mdi_testosterone, MDintegrationsPatientPharmacy
import time
...
def mdi_attach_pharmacy_to_patient(data: MDintegrationsPatientPharmacy):
    ...
    try:
        ...
        for mdi_api in mdi_instance_dict.values():
            res = mdi_api.get_patient(patient_id=data.patient_id)
            if 'patient_id' in res:
                api_instance = mdi_api
                break
        while True:
            attempt += 1
            res = api_instance.add_pharmacy_to_patient(
