...
def get_states(drivers, start=1):
    drivers.find_element(By.XPATH, '//*[@id="j_idt32_label"]').click()
    states = drivers.find_elements(By.XPATH, "//div[@id='j_idt32_panel']/div/ul/li")
    return states[start:]
...
def click_refresh_button(drivers):
    drivers.find_element(By.ID, 'j_idt62').click()
...
try:
    ...
    for state in all_states:
        ...
        for rto in all_rtos:
            ...
            sidebar_tables = driver.find_elements(By.XPATH, '//div[@id="j_idt68_content"]/div')
            for table in range(0, len(sidebar_tables), 2):
                print(table)
                table_items = driver.find_elements(By.XPATH, f'//div[@id="j_idt68_content"]/div[{table}]/div/div/div/table')
                print(table_items)
