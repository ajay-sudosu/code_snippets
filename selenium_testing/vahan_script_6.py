...
def select_fuel_type(driver, fuel_type):
    ...
    for fuel in fuel_list:
        if fuel.text == fuel_type:  # for CNG ONLY
            ...
            for fuel_type in data:
                ...
                for model in data[fuel_type]:
                    ...
                    if table_check.text == "No records found.":
                        ...
                        time.sleep(6)
                        file_path = os.path.join(BASE_DIR, state_name, rto_name)
                        file_name = f"{state_name.split('(')[0]}_{rto_name.split('-')[0]}_{model}_{fuel_type}.xlsx"
                        if not os.path.exists(file_path):
                            os.makedirs(file_path)
                        shutil.copy2(os.path.join(BASE_DIR, 'reportTable.xlsx'), os.path.join(file_path))
                        ...
                        shutil.copy2(os.path.join(file_path, file_name), os.path.join(BASE_DIR, "data"))
